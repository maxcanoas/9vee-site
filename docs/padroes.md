...
- A cor de ação é uma só: menta, nos botões e no atalho do WhatsApp. Violeta e magenta, puras ou nos tons do grifo, marcam grupo e escolha, nunca botão de ação.
...
- O texto sobre a faixa segue a regra dos 4,5:1, porque a faixa passa por trás das letras. O tom da faixa segue a cor desse texto: com texto navy, os tons claros (`--grifo-violeta-claro` e `--grifo-magenta-claro`); com texto papel, no `.escuro`, os tons escuros do `base.css`. A menta não vai sob texto papel. Em fundo escuro sem texto papel sobre a cor (a escolha de público, com texto navy, e as meias-luas da faixa de saudações, sem texto), a classe `.tons-claros` traz de volta os tons claros: contra o navy, eles dão 4,7:1 e 4,6:1, e os escuros só 2,7:1 e 2,8:1.
