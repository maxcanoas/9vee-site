...
- **Código de cor dos grupos:** cada público e cada família de idiomas tem a sua cor, que aparece em marca-texto no título do grupo. É uma faixa sólida atrás da parte de baixo das letras (a classe `.grifo`), com o texto na cor de sempre. Empresas fica em violeta e Para você em magenta, no menu do celular, no menu do computador (na meia-lua e, com o painel aberto, na faixa) e no rodapé. Germânicas fica em violeta, Românicas em magenta e De outras famílias em menta, na home e na página de cursos. As cores são tokens (`--cor-empresas`, `--cor-voce` e as das famílias).
